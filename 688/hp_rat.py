...
def hp_rat(bench_test, log_config, log_lock, response_que, econet_write_obj_que, econet_read_obj_que, kill_que):
    ...
    Test Case Purpose:  
        ...
        If the Return Air Temperature is below -40 deg F and a valid thermistor value was previously sensed, then "Sensor Fault 81" shall be logged. 
        If the Return Air Temperature is above 250 deg F and a valid thermistor value was previously sensed, then "Sensor Fault 72" shall be logged. 
        If the resistance is too low out of range, fault 81 should be active. 
        If the resistance is too high out of range, fault 72 should be active.
    ...
    The steps are as follows:
        ...
        2. Set XXX_TEMR to 0. Verify Control board sets fault 81 Return Air Temperature Sensor Fault
        3. Reset the microcontroller by setting RESETDEV to 1
        4. Set XXX_TEMR to 500k ohms and verify the alert 72 Return Air Temperature Sensor Fault is set.
    ...
    try:
        ...
        # Verify fault 81 is active due to the resistance and temperature being too low out of range
        if not bench_test:
            tc_logger.log_entry('***** Waiting 15 seconds for potential fault to set')
            time.sleep(15)
        tc_logger.log_entry('***** Verifying fault 81A is active')
        alarm_01 = read_obj(tc_logger, response_que, econet_read_obj_que, 'ALARM_01', NetworkAddresses.ECONET_XXXX)[1]
        if alarm_01[0:6] != "T081_A" and not bench_test: #Only want preform the raise this if alarm is not active and if we are not in a benchtest
            raise ValueCompareError(f'Fault T081_A not active')
        ...
        # Verify fault 72 is active due to the resistance and temperature being too high out of range
        if not bench_test:
            tc_logger.log_entry('***** Waiting 15 seconds for potential fault to set')
            time.sleep(15)
        tc_logger.log_entry('***** Verifying fault 72A is active')
        alarm_01 = read_obj(tc_logger, response_que, econet_read_obj_que, 'ALARM_01', NetworkAddresses.ECONET_XXXX)[1]
        if alarm_01[0:6] != "T072_A"  and not bench_test: #Only want preform the raise this if alarm is not active and if we are not in a benchtest
            raise ValueCompareError(f'Fault T072_A not active')
