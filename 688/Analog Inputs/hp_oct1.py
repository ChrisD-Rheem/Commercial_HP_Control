...
def hp_oct1(bench_test, log_config, log_lock, response_que, econet_write_obj_que, econet_read_obj_que, kill_que):
    ...
    Test Case Purpose:  
        ...
        If the Outdoor Coil Temperature #1 is below -40 deg F and a valid thermistor value was previously sensed, then "Sensor Fault 83" shall be logged. 
        If the Outdoor Coil Temperature #1 is above 250 deg F and a valid thermistor value was previously sensed, then "Sensor Fault 83" shall be logged. 
        If the resistance is too low out of range, fault 83 should be active. 
        If the resistance is too high out of range, fault 83 should be active.
    ...
    The steps are as follows:
        ...
        2. Set XXX_TEMR to 0. Verify Control board sets fault 83 Outdoor Coil Temperature #1 Sensor Fault
        3. Reset the microcontroller by setting RESETDEV to 1
        4. Set XXX_TEMR to 500k ohms and verify the alert 83 Outdoor Coil Temperature #1 Sensor Fault is set.
    ...
    try:
        ...
        if not bench_test:
            ...
            time.sleep(15)
        tc_logger.log_entry('***** Verifying fault 83A is active')
        ...
        if alarm_01[0:6] != "T0XX_A" and not bench_test: #Only want preform the raise this if alarm is not active and if we are not in a benchtest
            raise ValueCompareError(f'Fault T083_A not active')
