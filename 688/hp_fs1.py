...
def hp_fs1(bench_test, log_config, log_lock, response_que, econet_write_obj_que, econet_read_obj_que, kill_que):
    """
     Test Case Requirement:
    ACHPC-SR-207 - FreezeStat Sensor
    Test Case Purpose:  
        This will verify the operating temperature range and resistance range of the FreezeStat Sensor. 
        If the FreezeStat is below -40 deg F and a valid thermistor value was previously sensed, then "Sensor Fault XXXXXX" shall be logged. 
        If the FreezeStat is above 250 deg F and a valid thermistor value was previously sensed, then "Sensor Fault XXXXXX" shall be logged. 
        ...
        If resistance is set to 0 ohms, then the FreezeStat goes below -40 deg F.
        If resistance is set to 500k ohms, then the FreezeStat goes above 250 deg F.

    The steps are as follows:
        1. If board is not powered up, do so and wait approx. 20 sec for board to record the sensor readings. Read FS1_TEMP which should be around whatever room temperature is at the time.  
        2. Set FS1_TEMR to 0. Verify Base Control board sets fault XXXXXX_X(X) FreezeStat Sensor Fault
        3. Reset the microcontroller by setting RESETDEV to 1
        4. Set FS1_TEMR to 500k ohms and verify the alert XXXXXX_X FreezeStat Sensor Fault is set.
    ...
    try:
        ...
        # Verify FS1_TEMP reads room temperature , room temperature value stored in variable room_temp and a tolerance of 3 degrees is applied.
        ...
        read_obj_compare(tc_logger, response_que, econet_read_obj_que, "FS1_TEMP", room_temp, ControlValues.THERMISTOR_TOLERENCE, dest_addr=NetworkAddresses.ECONET_XXXX)

        # Step 2. Setting the value for FS1_TEMR to 0 ohms so that the resistance is too low out of range and this sets the temperature to below -40 deg F
        # which the reported temperature goes to -40 deg F. -40 deg F is the minimum FS1_TEMP value. LOW_FAIL_RESISTANCE is 0
        tc_logger.log_entry(f'***** Setting FreezeStat resistance to {ControlValues.LOW_FAIL_RESISTANCE}')
        write_obj(tc_logger, response_que, econet_write_obj_que, "FS1_TEMR", ControlValues.LOW_FAIL_RESISTANCE, NetworkAddresses.ECONET_XXXX)   
        ...
        # Step 4. Setting FS1_TEMR to 500000 ohms so that the resistance is too high out of range and this sets the temperature to above the
        # 250 deg F which the reported temperature goes to 250 deg F. 250 deg F is the maximum FS1_TEMP value. HIGH_FAIL_RESISTANCE is 500000 ohms
        tc_logger.log_entry(f'***** Setting FreezeStat resistance to {ControlValues.HIGH_FAIL_RESISTANCE}')
        write_obj(tc_logger, response_que, econet_write_obj_que, "FS1_TEMR", ControlValues.HIGH_FAIL_RESISTANCE, NetworkAddresses.ECONET_XXXX)
...
def main(bench_test = False):
    ...
    try:
        ...
        test_case_proc = Process(target=hp_fs1, name=TEST_CASE_NAME,
